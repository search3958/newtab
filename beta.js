...
function loadAdsenseScript() {
    return new Promise((resolve, reject) => {
        ...
        script.async = true;
        script.src = 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-6151036058675874';
...
function initAdsDeferred(container) {
        ...
        requestIdleCallback(() => {
            try {
                ...
                loadAdsenseScript().then(() => {
                    ...
                    console.warn('[Ads] ads initialization failed');
                });
            } catch (e) {
    ...
    const loadZip = async (url) => {
        try {
            ...
            for (let i = 0; i < entriesLen; i++) {
                ...
                imageMap[fileName] = URL.createObjectURL(new Blob([data.buffer], {
                    type: 'image/webp'
                }));
            }
            return imageMap;
    ...
    const loadData = async () => {
        ...
        try {
            const [imageMap, res] = await Promise.all([loadZip('lsr/icons-6-2.zip'), fetch('links-v6.json')]);
            ...
            for (let idx = 0; idx < catLen; idx++) {
                ...
                for (let j = 0; j < linksLen; j++) {
                    ...
                    const originalBg = link.bg || '#eee';
                    const iconMode = parseInt(localStorage.getItem('icon_mode')) || 0;
                    appDiv.dataset.originalBg = originalBg;
                    appDiv.style.background = iconMode === 1 ? 'var(--iconbg)' : originalBg;
...
requestIdleCallback(() => {
    ...
    script.setAttribute('data-xml', 'https://search3958.github.io/newtab/xml/beta.xml');
